...
class KerberosContext(context.OSContextGenerator):
    interfaces = ['domain-backend']
    ...
    def __call__(self):
        ...
        for rid in relation_ids('domain-backend'):
            if related_units(rid):
                for unit in related_units(rid):
                    kerberos_enabled = relation_get(attribute='kerberos-enabled',
                                               unit=unit,
                                               rid=rid)
                    if kerberos_enabled:
                        kerberos_info['kerberos_enabled'] = kerberos_enabled
                        log(kerberos_info)
                        kerberos_realm = relation_get(attribute='kerberos-realm',
                                                   unit=unit,
                                                   rid=rid)
                        kerberos_info['kerberos_realm'] = kerberos_realm
                        keytab_path = relation_get(attribute='keytab-path',
                                                   unit=unit,
                                                   rid=rid)
                        kerberos_info['keytab_path'] = keytab_path
                        domain = relation_get(attribute='domain-name',
                                                   unit=unit,
                                                   rid=rid)
                        kerberos_info['domain'] = domain
                        log(kerberos_info)
            ...
            if related_units(rid):
                for unit in related_units(rid):
                    kerberos_enabled = relation_get(attribute='kerberos-enabled',
                                               unit=unit,
                                               rid=rid)
                    if kerberos_enabled and "kerberos" not in self.auth_methods:
                        self.auth_methods.append("kerberos")
